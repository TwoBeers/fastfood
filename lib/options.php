...
class FastfoodOptions {
		...
		self::$coa = apply_filters( 'fastfood_options_array', array(
			...
			'fastfood_basic_animations' =>
							array(
								...
								'info'				=> '',
								'req'				=> 'fastfood_jsani',
								'sub'				=> array( 'fastfood_basic_animation_main_menu', 'fastfood_basic_animation_navigation_buttons', 'fastfood_basic_animation_quickbar_panels', 'fastfood_basic_animation_entry_meta', 'fastfood_basic_animation_smooth_scroll', 'fastfood_basic_animation_captions' ),
							),
			...
			'fastfood_comments_navigation' =>
							array(
								'type'				=> 'chk',
								'default'			=> 0,
								'description'		=> __( 'comments navigation', 'fastfood' ),
								'info'				=> __( 'ajaxed navigation for comments', 'fastfood' ),
								'req'				=> 'fastfood_jsani',
							),
			...
			'fastfood_rsideb' =>
							array(
								...
								'info'				=> '',
								'req'				=> '',
								'sub'				=> array( 'fastfood_rsidebindexes', 'fastfood_rsidebpages', 'fastfood_rsidebposts', 'fastfood_rsidebattachments' ),
							),
			...
			'fastfood_rsidebattachments' =>
							array(
								'type'				=> 'chk',
								'default'			=> 0,
								'description'		=> __( 'in attachments', 'fastfood' ),
								'info'				=> '',
								'req'				=> '',
								'sub'				=> false,
							),
			...
			'fastfood_primary_menu' =>
							array(
								...
								'info'				=> '',
								'req'				=> '',
								'sub'				=> array( 'fastfood_sticky_menu' ),
							),
			'fastfood_sticky_menu' =>
							array(
								'type'				=> 'chk',
								'default'			=> 0,
								'description'		=> __( 'sticky', 'fastfood' ),
								'info'				=> __( 'the menu will stick on top when the user scrolls down the page', 'fastfood' ),
								'req'				=> '',
								'sub'				=> false,
							),
			...
			'fastfood_google_font_family'=>
							array(
								...
								'info'				=> sprintf( __( 'Copy and paste here the name of a font from the %s library . Example: %s', 'fastfood' ), '<a href="http://www.google.com/webfonts" target="_blank"><strong>Google web font</strong></a>', '<code>Droid Sans</code>' ),
								'req'				=> '',
	...
	*/
	public static function get_coa( $option = false, $data = false ) {
		...
		if ( $option )
			if ( $data )
				return isset( self::$coa[$option][$data] ) ? self::$coa[$option][$data] : false;
			else
	...
	*/
	 ...
	 * If the the option doesn't exists, return null
	 *
	 ...
	 */
	public static function get_opt( $opt, $fallback = NULL ) {
		global $fastfood_opt;

		if ( isset( $fastfood_opt[$opt] ) ) return apply_filters( 'fastfood_option_' . $opt, $fastfood_opt[$opt], $opt );

		$defopt = self::get_coa( $opt );

		if ( ! $defopt ) return $fallback;

		if ( ( $defopt['req'] == '' ) || ( self::get_opt( $defopt['req'] ) ) )
	...
	/**
	 * Shortcut function for adding a custom option
	 *
	 * Add an option to the default ones
	 * Option MUST HAVE either a position (tab and group) OR a parent option
	 * Cannot modify existing options, if you need to modify the option array use the filter 'fastfood_options_array'
	 *
	 * @param	array	$options	(required) the option to add
	 * @param	array	$position	(optional) the position in the theme options page - array( tab, group )
	 * @param	string	$parent		(optional) the name of the parent option (for suboptions)
	 * @return	none
	 */
	public static function add_option( $options, $position = false, $parent = false ) {

		foreach ( $options as $key => $option) {

			if ( isset( self::$coa[$key] ) ) return;

			$has_location = false;

			if ( $parent ) {
				if ( !isset( self::$coa[strval( $parent )] ) ) return;
				self::$coa[strval( $parent )]['sub'][] = $key;
				$has_location = true;
			} elseif ( $position && is_array( $position ) && count( $position ) == 2 ) {
				if ( !isset( self::$hierarchy[strval( $position[0] )] ) ) return;
				self::$hierarchy[strval( $position[0] )]['sub'][strval( $position[1] )]['sub'][] = $key;
				$has_location = true;
			}

			if ( $has_location ) self::$coa[$key] = $option;

 ...
 * Fastfood Widgets
 *
 * based on WordPress default widgets
 */
		...
		<li>
			<?php 			
				if( has_post_thumbnail() ) {
					...
					// echo '<img height="50" width="50" src="' . get_template_directory_uri() . '/images/thumb_50.png" alt="thumb" />';
...
<?php
	...
	function form( $instance ) {
		...
		if ( !isset($instance['number']) || !$number = (int) $instance['number'] )
			$number = 5;
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','fastfood'); ?></label>
...
/**
 * Last_Commented_Posts widget class
 *
 */
class fastfood_Widget_Last_Commented_Posts extends WP_Widget {

	function fastfood_Widget_Last_Commented_Posts() {
		$widget_ops = array('classname' => 'ff_widget_last_commented_posts', 'description' => __( '[by Fastfood theme] The last commented posts/pages of your site','fastfood' ) );
		$this->WP_Widget('ff-recent-comments', __('Last activity','fastfood'), $widget_ops);
		$this->alt_option_name = 'ff_widget_last_commented_posts';
	...
	function flush_widget_cache() {
		wp_cache_delete('ff_widget_last_commented_posts', 'widget');
	}

	function widget( $args, $instance ) {
		global $comments, $comment;

		$cache = wp_cache_get('ff_widget_last_commented_posts', 'widget');
		...
		if ( isset( $cache[$args['widget_id']] ) ) {
 		...
 		$output = '';
 		$title = apply_filters('widget_title', empty($instance['title']) ? __('Last activity','fastfood') : $instance['title']);
		...
		wp_cache_set('ff_widget_last_commented_posts', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		$alloptions = wp_cache_get( 'alloptions', 'options' );
		if ( isset($alloptions['ff_widget_last_commented_posts']) )
			delete_option('ff_widget_last_commented_posts');
...
function fastfood_widgets_init() {
	...
	register_widget('fastfood_Widget_Last_Commented_Posts');
}
